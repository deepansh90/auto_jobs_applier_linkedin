from __future__ import annotations

# Imports

import os
import sys
import json
import pathlib
import subprocess
import re

from time import sleep
from random import randint
from datetime import datetime, timedelta
from pprint import pprint

from config.settings import logs_folder_path



#### Common functions ####

#< Directories related
def make_directories(paths: list[str]) -> None:
    '''
    Function to create missing directories
    '''
    for path in paths:
        path = os.path.expanduser(path) # Expands ~ to user's home directory
        path = os.path.normpath(path)
        
        # If path looks like a file path, get the directory part
        if '.' in os.path.basename(path):
            path = os.path.dirname(path)

        if not path: # Handle cases where path is empty after dirname
            continue

        try:
            if not os.path.exists(path):
                os.makedirs(path, exist_ok=True) # exist_ok=True avoids race condition
        except Exception as e:
            print(f'Error while creating directory "{path}": ', e)


def get_default_temp_profile() -> str:
    # Thanks to https://github.com/vinodbavage31 for suggestion!
    home = pathlib.Path.home()
    if sys.platform.startswith('win'):
        return "--user-data-dir=C:\\temp\\auto-job-apply-profile"
    elif sys.platform.startswith('linux'):
        return str(home / ".auto-job-apply-profile")
    return str(home / "Library" / "Application Support" / "Google" / "Chrome" / "auto-job-apply-profile")


def get_chromium_temp_profile() -> str:
    '''
    Dedicated user-data directory for Chromium guest / safe-mode runs
    (separate from Google Chrome temp profile).
    '''
    home = pathlib.Path.home()
    if sys.platform.startswith('win'):
        return str(pathlib.Path(os.path.expandvars(r"%LOCALAPPDATA%")) / "Chromium" / "auto-job-apply-profile")
    if sys.platform.startswith('linux'):
        return str(home / ".config" / "chromium-auto-job-apply-profile")
    return str(home / "Library" / "Application Support" / "Chromium" / "auto-job-apply-profile")


def find_chromium_user_data_directory() -> str | None:
    '''
    Returns the Chromium user-data root (same role as find_default_profile_directory for Chrome).
    '''
    home = pathlib.Path.home()
    if sys.platform.startswith('win'):
        p = pathlib.Path(os.path.expandvars(r"%LOCALAPPDATA%")) / "Chromium" / "User Data"
    elif sys.platform.startswith('linux'):
        p = home / ".config" / "chromium"
    elif sys.platform == 'darwin':
        p = home / "Library" / "Application Support" / "Chromium"
    else:
        return None
    return str(p) if p else None


def resolve_chromium_binary_path(explicit_path: str) -> str | None:
    '''
    Resolves the Chromium executable path. If explicit_path is non-empty and valid, use it.
    Otherwise try common install locations (macOS Homebrew cask, Linux distro packages).
    '''
    home = pathlib.Path.home()
    candidates: list[str] = []
    if explicit_path and explicit_path.strip():
        ep = explicit_path.strip()
        if os.path.isfile(ep):
            return ep
        candidates.append(ep)
    candidates.extend(
        [
            "/Applications/Chromium.app/Contents/MacOS/Chromium",
            str(home / "Applications" / "Chromium.app" / "Contents" / "MacOS" / "Chromium"),
            "/opt/homebrew/bin/chromium",
            "/usr/local/bin/chromium",
            "/usr/bin/chromium",
            "/usr/bin/chromium-browser",
        ]
    )
    for path_str in candidates:
        if path_str and os.path.isfile(path_str) and os.access(path_str, os.X_OK):
            return path_str
    return None


def browser_major_version_from_binary(binary_path: str) -> int | None:
    '''
    Parses major version from `binary_path --version` (Chrome or Chromium).
    '''
    try:
        output = subprocess.check_output(
            [binary_path, "--version"], shell=False, text=True, timeout=15, stderr=subprocess.STDOUT
        )
        match = re.search(r"(?:Chromium|Chrome) (\d+)\.", output)
        if match:
            return int(match.group(1))
    except Exception as e:
        print(f"Error detecting browser version from {binary_path}: {e}")
    return None


def find_default_profile_directory() -> str | None:
    '''
    Dynamically finds the default Google Chrome 'User Data' directory path
    across Windows, macOS, and Linux, regardless of OS version.

    Returns the absolute path as a string, or None if the path is not found.
    '''
    
    home = pathlib.Path.home()
    
    # Windows
    if sys.platform.startswith('win'):
        paths = [
            os.path.expandvars(r"%LOCALAPPDATA%\Google\Chrome\User Data"),
            os.path.expandvars(r"%USERPROFILE%\AppData\Local\Google\Chrome\User Data"),
            os.path.expandvars(r"%USERPROFILE%\Local Settings\Application Data\Google\Chrome\User Data")
        ]
    # Linux
    elif sys.platform.startswith('linux'):
        paths = [
            str(home / ".config" / "google-chrome"),
            str(home / ".var" / "app" / "com.google.Chrome" / "data" / ".config" / "google-chrome"),
        ]
    # MacOS: use the dedicated bot profile, NOT Chrome's main User Data dir (which conflicts with the user's Chrome)
    elif sys.platform == 'darwin':
        paths = [
            str(home / "Library" / "Application Support" / "Google" / "Chrome" / "auto-job-apply-profile")
        ]
    else:
        return None

    # Check each potential path and return the first one that exists
    for path_str in paths:
        if os.path.exists(path_str):
            return path_str

    # On darwin we use a dedicated bot profile — create it if missing so the bot
    # never silently falls back to guest mode (which loses login cookies).
    if sys.platform == 'darwin' and paths:
        os.makedirs(paths[0], exist_ok=True)
        return paths[0]

    return None
#>


#< Logging related
def critical_error_log(possible_reason: str, stack_trace: Exception) -> None:
    '''
    Function to log and print critical errors along with datetime stamp
    '''
    print_lg(possible_reason, stack_trace, datetime.now(), from_critical=True)


def get_log_path():
    '''
    Return the path for this run's session log.

    Writes to `logs/session_YYYYMMDD.log` (one file per calendar day) so each
    run keeps its own history rather than endlessly appending to a single
    `log.txt`. For backward compatibility we also maintain a `logs/log.txt`
    symlink-style copy by letting callers grep the session file directly; any
    docs/tooling still pointing at `log.txt` should be updated to the session
    file name instead.
    '''
    try:
        stamp = datetime.now().strftime("%Y%m%d")
        path = f"{logs_folder_path}/session_{stamp}.log"
        return path.replace("//", "/")
    except Exception as e:
        critical_error_log(
            "Failed getting log path! So assigning default logs path: "
            "'./logs/session.log'", e
        )
        return "logs/session.log"


__logs_file_path = get_log_path()


def print_lg(*msgs: str | dict, end: str = "\n", pretty: bool = False, flush: bool = False, from_critical: bool = False) -> None:
    '''
    Function to log and print. **Note that, `end` and `flush` parameters are ignored if `pretty = True`**
    '''
    try:
        for message in msgs:
            pprint(message) if pretty else print(message, end=end, flush=flush)
            with open(__logs_file_path, 'a+', encoding="utf-8") as file:
                file.write(str(message) + end)
    except Exception as e:
        try:
            sys.stderr.write(f"[print_lg] logging failed: {e}\n")
        except Exception:
            pass
        trail = f'Skipped saving this message: "{message}" to log.txt!' if from_critical else "We'll try one more time to log..."
        try:
            smart_alert(
                f"log.txt in {logs_folder_path} is open or is occupied by another program! Please close it! {trail}",
                "Failed Logging",
            )
        except Exception as alert_err:
            try:
                sys.stderr.write(f"[print_lg] smart_alert failed: {alert_err}\n")
            except Exception:
                pass
        if not from_critical:
            critical_error_log("Log.txt is open or is occupied by another program!", e)
#>


def buffer(speed: int=0) -> None:
    '''
    Function to wait within a period of selected random range.
    * Will not wait if input `speed <= 0`
    * Will wait within a random range of 
      - `0.6 to 1.0 secs` if `1 <= speed < 2`
      - `1.0 to 1.8 secs` if `2 <= speed < 3`
      - `1.8 to speed secs` if `3 <= speed`
    '''
    if speed<=0:
        return
    elif speed <= 1 and speed < 2:
        return sleep(randint(6,10)*0.1)
    elif speed <= 2 and speed < 3:
        return sleep(randint(10,18)*0.1)
    else:
        return sleep(randint(18,round(speed)*10)*0.1)
    

def manual_login_retry(is_logged_in: callable, limit: int = 2) -> None:
    '''
    Function to ask and validate manual login
    '''
    count = 0
    while not is_logged_in():
        print_lg("Seems like you're not logged in!")
        button = "Confirm Login"
        message = 'After you successfully Log In, please click "{}" button below.'.format(button)
        if count > limit:
            button = "Skip Confirmation"
            message = 'If you\'re seeing this message even after you logged in, Click "{}". Seems like auto login confirmation failed!'.format(button)
        count += 1
        if smart_alert(message, "Login Required", button) and count > limit: return



def calculate_date_posted(time_string: str) -> datetime | None:
    '''
    Function to calculate date posted from string.
    Returns datetime object, or None if unable to calculate.
    Valid time string examples:
    * 10 seconds ago
    * 15 minutes ago
    * 2 hours ago
    * 1 hour ago
    * 1 day ago
    * 10 days ago
    * 1 week ago
    * 1 month ago
    * 1 year ago
    '''
    import re
    time_string = time_string.strip()
    now = datetime.now()

    match = re.search(r'(\d+)\s+(second|minute|hour|day|week|month|year)s?\s+ago', time_string, re.IGNORECASE)

    if match:
        try:
            value = int(match.group(1))
            unit = match.group(2).lower()

            if 'second' in unit:
                return now - timedelta(seconds=value)
            elif 'minute' in unit:
                return now - timedelta(minutes=value)
            elif 'hour' in unit:
                return now - timedelta(hours=value)
            elif 'day' in unit:
                return now - timedelta(days=value)
            elif 'week' in unit:
                return now - timedelta(weeks=value)
            elif 'month' in unit:
                return now - timedelta(days=value * 30)  # Approximation
            elif 'year' in unit:
                return now - timedelta(days=value * 365)  # Approximation
        except (ValueError, IndexError):
            # Fallback for cases where parsing fails
            pass
    
    # If regex doesn't match, or parsing failed, return None.
    # This will skip jobs where the date can't be determined, preventing crashes.
    return None


def convert_to_lakhs(value: str) -> str:
    '''
    Converts a numeric string to a "lakhs-style" display (digits before last five = lakhs, next two = fractional).
    Non-digits (commas, spaces, currency symbols) are stripped before formatting.
    Examples:
    * "100000" -> "1.00"
    * "101000" -> "1.01"
    * "5000" -> "0.050"
    '''
    raw = value.strip()
    digits = re.sub(r"\D", "", raw)
    if not digits:
        return raw
    value = digits
    l = len(value)
    if l > 0:
        if l > 5:
            value = value[:l-5] + "." + value[l-5:l-3]
        else:
            value = "0." + "0"*(5-l) + value[:2]
    return value


def convert_to_json(data) -> dict:
    '''
    Function to convert data to JSON, if unsuccessful, returns `{"error": "Unable to parse the response as JSON", "data": data}`
    '''
    try:
        result_json = json.loads(data)
        return result_json
    except json.JSONDecodeError:
        return {"error": "Unable to parse the response as JSON", "data": data}


def truncate_for_csv(data, max_length: int = 950_000, suffix: str = "...[TRUNCATED]") -> str:
    '''
    Function to truncate data for CSV writing to avoid field size limit errors.
    * Takes in `data` of any type and converts to string
    * Takes in `max_length` of type `int` - maximum allowed length (default aligned with raised csv field limit)
    * Takes in `suffix` of type `str` - text to append when truncated
    * Returns truncated string if data exceeds max_length
    '''
    try:
        # Convert data to string
        str_data = str(data) if data is not None else ""
        
        # If within limit, return as-is
        if len(str_data) <= max_length:
            return str_data
        
        # Truncate and add suffix
        truncated = str_data[:max_length - len(suffix)] + suffix
        return truncated
    except Exception as e:
        return f"[ERROR CONVERTING DATA: {e}]"


def get_chrome_major_version() -> int | None:
    '''
    Detects the major version of Google Chrome or Chromium (when use_chromium is True in settings).
    Returns the major version as an integer, or None if detection fails.
    '''
    from config.settings import use_chromium, chromium_binary_path

    if use_chromium:
        binary_path = resolve_chromium_binary_path(chromium_binary_path or "")
        if binary_path:
            return browser_major_version_from_binary(binary_path)
        return None
    try:
        if sys.platform.startswith('win'):
            output = subprocess.check_output(
                [
                    "reg",
                    "query",
                    r"HKEY_CURRENT_USER\Software\Google\Chrome\BLBeacon",
                    "/v",
                    "version",
                ],
                shell=False,
                text=True,
                timeout=15,
            )
            version = re.search(r"(\d+)\.", output)
            if version:
                return int(version.group(1))
        elif sys.platform == 'darwin':
            chrome_bin = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
            output = subprocess.check_output(
                [chrome_bin, "--version"], shell=False, text=True, timeout=15, stderr=subprocess.STDOUT
            )
            version = re.search(r"Chrome (\d+)\.", output)
            if version:
                return int(version.group(1))
        elif sys.platform.startswith('linux'):
            output = subprocess.check_output(
                ["google-chrome", "--version"], shell=False, text=True, timeout=15, stderr=subprocess.STDOUT
            )
            version = re.search(r"Chrome (\d+)\.", output)
            if version:
                return int(version.group(1))
    except Exception as e:
        print(f"Error detecting Chrome version: {e}")
    return None



# Global flag for Tkinter availability
_TKINTER_AVAILABLE = None

def is_tkinter_available() -> bool:
    global _TKINTER_AVAILABLE
    if _TKINTER_AVAILABLE is None:
        try:
            import tkinter
            # Test if a window can actually be opened (prevents headless environment crashes)
            root = tkinter.Tk()
            root.withdraw()
            root.destroy()
            _TKINTER_AVAILABLE = True
        except Exception:
            _TKINTER_AVAILABLE = False
    return _TKINTER_AVAILABLE

def smart_alert(text: str, title: str = "Alert", button: str = "OK") -> bool:
    '''
    Shows an alert message. Tries GUI first, falls back to terminal.
    Returns True to indicate that the alert was 'acknowledged'.
    '''
    if is_tkinter_available():
        try:
            from pyautogui import alert
            alert(text, title, button)
            return True
        except Exception:
            pass
    
    # Fallback to console
    print(f"\n[{title}]\n{text}\nPress Enter to continue: [{button}]", end="")
    try:
        input()
    except EOFError:
        print_lg(f"Warning: Console input not available for alert: {text}")
    return True


def smart_confirm(text: str, title: str = "Confirm", buttons: list[str] = ["OK", "Cancel"]) -> str:
    '''
    Shows a confirmation message. Tries GUI first, falls back to terminal.
    Returns the string value of the button clicked.
    '''
    if is_tkinter_available():
        try:
            from pyautogui import confirm
            return confirm(text, title, buttons)
        except Exception:
            pass
            
    # Fallback to console
    print(f"\n[{title}]\n{text}")
    options_str = "/".join(buttons)
    while True:
        try:
            choice = input(f"Please type one of ({options_str}) [Default: {buttons[0]}]: ").strip()
            # Try to match choice with buttons (case-insensitive)
            if not choice:
                return buttons[0]
            for b in buttons:
                if choice.lower() == b.lower():
                    return b
            print(f"Invalid input. Please choose from {buttons}.")
        except EOFError:
            print_lg(f"Warning: Console input not available for confirm: {text}")
            return buttons[0] # Default to first option


def smart_prompt(text: str, title: str = "Prompt", default: str = "") -> str:
    '''
    Shows a prompt for input. Tries GUI first, falls back to terminal.
    Returns the input string.
    '''
    if is_tkinter_available():
        try:
            from pyautogui import prompt

            return prompt(text, title, default)
        except Exception:
            pass
    print(f"\n[{title}]\n{text}")
    if default:
        print(f"(Default: {default})")
    res = input("> ").strip()
    return res if res else default