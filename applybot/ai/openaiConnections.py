...
def ai_error_alert(message: str, stackTrace: str, title: str = "AI Connection Error") -> None:
    ...
    if showAiErrorAlerts:
        if "Pause AI error alerts" == smart_confirm(f"{message}{stackTrace}\n", title, ["Pause AI error alerts", "Okay Continue"]):
...
def ai_create_openai_client(api_key: str = None, base_url: str = None, model_name: str = None) -> OpenAI:
    """
    Function to create an OpenAI client.
    * Takes optional `api_key`, `base_url`, and `model_name`
    * Returns an `OpenAI` object
    ...
    try:
        ...
        # Use provided credentials or fall back to globals
        target_key = api_key or llm_api_key
        target_url = base_url or llm_api_url
        target_model = model_name or llm_model

        client = OpenAI(base_url=target_url, api_key=target_key)
        ...
        if target_model not in available_model_ids:
            # If the specific model isn't found, try to find a sensible fallback or just log warning
            print_lg(f"Warning: Model `{target_model}` not found in available models: {available_model_ids[:5]}...")
        
        print_lg("---- SUCCESSFULLY CREATED OPENAI CLIENT! ----")
        print_lg(f"Using API URL: {target_url}")
        print_lg(f"Using Model: {target_model}")
        print_lg("---------------------------------------------")
...
def ai_answer_question(
    ...
    try:
        ...
        messages = [{"role": "user", "content": prompt}]
        # Do not log full prompt: it contains résumé-derived PII (user_information_all).
        print_lg(f"Prompt passed to AI ({len(prompt)} chars, contents not logged).")
...
def ai_generate_resume(
    ...
    '''
    Function to generate tailored resume content.
    '''
    print_lg("-- TAILORING RESUME using AI")
    try:
        prompt = tailor_resume_prompt.format(master_resume, job_description)
        messages = [{"role": "user", "content": prompt}]
        return ai_completion(client, messages, response_format=tailor_resume_response_format, stream=stream)
    except Exception as e:
        ai_error_alert(f"Error occurred while tailoring resume. {apiCheckInstructions}", e)
...
def ai_check_job_relevance(
    ...
    '''
    Function to check job relevance and assign a match score.
    '''
    print_lg("-- CHECKING JOB RELEVANCE using AI")
    try:
        prompt = job_relevance_prompt.format(master_resume, job_description)
        messages = [{"role": "user", "content": prompt}]
        return ai_completion(client, messages, response_format=job_relevance_response_format, stream=stream)
    except Exception as e:
