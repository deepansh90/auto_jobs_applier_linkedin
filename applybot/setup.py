...
def parse_resume():
    ...
    with open(os.path.join(config_dir, "secrets.py"), "w", encoding="utf-8") as f:
        f.write(f'use_AI = True\nai_provider = {json.dumps(ai_prov)}\n')
        if ai_prov == "gemini":
            f.write(f'GEMINI_API_KEY = {json.dumps(key)}\nllm_api_key = {json.dumps(key)}\n')
        else:
            f.write(f'OPENAI_API_KEY = {json.dumps(key)}\nllm_api_key = {json.dumps(key)}\n')
...
username = {json.dumps(data.get('li_username', ''))}
password = {json.dumps(data.get('li_password', ''))}
use_AI = True
ai_provider = {json.dumps(ai_prov)}
'''
    if ai_prov == "gemini":
        secrets_content += f'GEMINI_API_KEY = {json.dumps(key)}\nOPENAI_API_KEY = ""\nllm_api_key = {json.dumps(key)}\n'
    else:
        secrets_content += f'OPENAI_API_KEY = {json.dumps(key)}\nGEMINI_API_KEY = ""\nllm_api_key = {json.dumps(key)}\n'
    ...
    try:
        desired_salary = int(data.get("desired_salary") or 7500000)
    except ValueError:
        return jsonify({"status": "error", "error": "Desired salary must be a number"}), 400
